...
class WorkPage extends Page {
	...
	private $statuses = [
		Entry::STATUS_0 => 'Планира се',
		Entry::STATUS_1 => 'Сканира се',
		Entry::STATUS_2 => 'За корекция',
		Entry::STATUS_3 => 'Коригира се',
		Entry::STATUS_4 => 'Иска се SFB',
		Entry::STATUS_5 => 'Чака проверка',
		Entry::STATUS_6 => 'Проверен',
		Entry::STATUS_7 => 'За добавяне',
	];
	private $viewLists = [
	...
	protected function processSubmission() {
		...
		switch ($this->workType) {
			case Entry::TYPE_SINGLE_USER: return $this->updateMainUserData();
			case Entry::TYPE_MULTI_USER: return $this->updateMultiUserData();
		}
	}
	...
	private function updateMainUserData() {
		...
		$set = [
			'id' => $id,
			'type' => in_array($this->status, [Entry::STATUS_4]) ? 1 : $this->workType,
	...
	protected function updateMultiUserDataForEdit() {
		...
		$set = [
			'date' => $this->date,
			'status' => $this->entry->hasOpenContribs() ? Entry::STATUS_3 : ( $this->isReady() ? Entry::STATUS_6 : Entry::STATUS_5 ),
		];
...
HTML;
	...
	private function makeForm() {
		...
		if ( $this->thisUserCanDeleteEntry() ) {
			...
			if ($this->status == Entry::STATUS_7 && !$this->userCanSetStatus(Entry::STATUS_7)) {
				$button = $delete = '';
			}
		} else {
			$title = $this->btitle;
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<label class="radio-inline">
						<input type="radio" name="packType" {$printPackTypeRadioValue(Entry::PACK_TYPE_BOOK)}> Книга
					</label>
					<label class="radio-inline">
						<input type="radio" name="packType" {$printPackTypeRadioValue(Entry::PACK_TYPE_WORK)}> Произведение
					</label>
				</div>
			</div>
			<div class="form-group">
...
FIELDS;
	...
	private function getStatusSelectField($selected, $max = null) {
		$statuses = $this->statuses;
		foreach ([Entry::STATUS_6, Entry::STATUS_7] as $status) {
...
EOS;
	...
	private function isMyContribAllowed() {
		...
		if (in_array($this->status, [Entry::STATUS_5, Entry::STATUS_6, Entry::STATUS_7])) {
			return false;
		}
...
EOS;
	...
	private function isSingleUser() {
		return $this->workType == Entry::TYPE_SINGLE_USER;
	}
	private function isMultiUser() {
		return $this->workType == Entry::TYPE_MULTI_USER;
	}

	private function thisUserCanEditEntry(Entry $entry, $type) {
		if ($this->user->isAnonymous()) {
			return false;
		}
		if ($this->userIsSupervisor() || $type == Entry::TYPE_MULTI_USER) {
			return true;
		}
...
EOS;
	...
	/** @return \App\Entity\EntryRepository */
